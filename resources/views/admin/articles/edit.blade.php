                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('admin.articles.update', $article->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title', $article->title) }}" class="mt-1 block w-full" required>
                        </div>

                        <div class="mb-4">
                            <label for="slug" class="block text-sm font-medium text-gray-700">Slug</label>
                            <input type="text" name="slug" id="slug" value="{{ old('slug', $article->slug) }}" class="mt-1 block w-full">
                            <p class="mt-1 text-xs text-gray-500">Kosongkan agar dibuat otomatis dari judul. Jika diisi, pastikan unik.</p>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="content" id="content" rows="10" class="mt-1 block w-full">{{ old('content', $article->content) }}</textarea>
                        </div>

                        <div class="mb-4">
                            <label for="excerpt" class="block text-sm font-medium text-gray-700">Kutipan Singkat (Excerpt - Opsional)</label>
                            <textarea name="excerpt" id="excerpt" rows="3" class="mt-1 block w-full">{{ old('excerpt', $article->excerpt) }}</textarea>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            @if ($article->featured_image_path)
                                <div class="mt-2">
                                    <img src="{{ asset('storage/' . $article->featured_image_path) }}" alt="{{ $article->title }}" class="h-20 w-20 object-cover rounded">
                                    <p class="text-xs text-gray-500">Gambar saat ini. Kosongkan jika tidak ingin diubah.</p>
                                </div>
                            @endif
                        </div>

                        {{-- Author biasanya tidak diubah, jika perlu bisa ditambahkan --}}

                        <div>
                            <button type="submit" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">Update Artikel</button>
