            ...
            <div class="md:col-span-2 bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                        ...
                        <div><strong>Pelanggan:</strong> {{ $serviceOrder->customer ? $serviceOrder->customer->name : 'N/A' }}</div>
                        <div><strong>No. HP Pelanggan:</strong> {{ $serviceOrder->customer && $serviceOrder->customer->phone_number ? $serviceOrder->customer->phone_number : '-' }}</div>
                        <div><strong>Tgl Diterima:</strong> {{ $serviceOrder->date_received ? $serviceOrder->date_received->format('d M Y, H:i') : '-' }}</div>
                        <div><strong>Estimasi Selesai (dari Admin):</strong> {{ $serviceOrder->estimated_completion_date ? \Carbon\Carbon::parse($serviceOrder->estimated_completion_date)->format('d M Y') : '-' }}</div>
                    </div>
                    ...
                    <p><strong>Status Persetujuan Pelanggan:</strong> {{ $serviceOrder->customer_approval_status ?: '-' }}</p>
                </div>
            </div>
                    ...
                    <div class="p-6 text-gray-900">
                        ...
                        @if ($errors->store_technician_update && $errors->store_technician_update->any())
                            <div class="mb-4 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">
                                ...
                                <ul>@foreach ($errors->store_technician_update->all() as $error)<li>{{ $error }}</li>@endforeach</ul>
                            </div>
                        @endif

                        {{-- Rute ini akan kita buat methodnya nanti: teknisi.service-orders.updates.store --}}
                        <form action="{{ route('teknisi.service-orders.updates.store', $serviceOrder->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="mb-4">
                                <label for="update_notes" class="block text-sm font-medium text-gray-700">Catatan Pengerjaan/Update</label>
                                <textarea name="notes" id="update_notes" rows="4" class="mt-1 block w-full" required>{{ old('notes') }}</textarea>
                            </div>

                            <div class="mb-4">
                                <label for="new_status" class="block text-sm font-medium text-gray-700">Ubah Status Order Ke (Opsional)</label>
                                <select name="new_status" id="new_status" class="mt-1 block w-full">
                                    <option value="">-- Jangan Ubah Status --</option>
                                    @foreach ($statuses as $value => $label) {{-- $statuses dari controller --}}
                                        <option value="{{ $value }}" {{ old('new_status') == $value ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>

                            <div class="mb-4">
                                <label for="photos" class="block text-sm font-medium text-gray-700">Unggah Foto Bukti (Opsional, bisa multiple)</label>
                            ...
                            <div>
                                <button type="submit" class="bg-sky-500 hover:bg-sky-700 text-white font-bold py-2 px-4 rounded w-full">
                        ...
                        @if($serviceOrder->updates && $serviceOrder->updates->count() > 0)
                            <div class="space-y-4">
                                ...
                                <div class="border rounded-md p-3 text-sm">
                                    <p class="font-semibold">{{ $update->update_type ?: 'Update' }} - <span class="font-normal text-gray-600">{{ $update->created_at->format('d M Y, H:i') }}</span></p>
                                    @if($update->updatedBy)
                                    ...
                                    @if($update->photos && $update->photos->count() > 0)
                                        <div class="mt-2 grid grid-cols-3 gap-2">
                                            ...
                                            <a href="{{ asset('storage/' . $photo->file_path) }}" data-lightbox="order-{{$serviceOrder->id}}-update-{{$update->id}}" data-title="{{ $photo->caption ?: $update->notes }}">
                                                <img src="{{ asset('storage/' . $photo->file_path) }}" alt="{{ $photo->caption ?: 'Bukti Foto' }}" class="h-16 w-full object-cover rounded">
                                            </a>
                                            @endforeach
                                        </div>
                                    @endif
