    ...
    <div class="container mx-auto mt-10 mb-10 p-4 max-w-3xl">
        <div class="text-center mb-6">
            <a href="{{ route('tracking.form') }}">
                <img src="{{ asset('images/logo_pangkalan_komputer.png') }}" alt="Logo Pangkalan Komputer ID"
                    class="mx-auto h-16 mb-2">
            </a>
            <h1 class="text-2xl font-semibold text-gray-700">Status Servis: {{ $serviceOrder['service_order_number'] }}
            </h1>
            <a href="{{ route('tracking.form') }}" class="text-sm text-indigo-600 hover:text-indigo-900">&larr; Lacak
                Nomor Servis Lain</a>
        </div>
        ...
        <div class="bg-white p-6 sm:p-8 rounded-lg shadow-lg space-y-6">
            ...
            <section>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 text-sm">
                    ...
                    <div><strong>Tgl Diterima:</strong> {{ $serviceOrder['date_received'] ?
                        \Carbon\Carbon::parse($serviceOrder['date_received'])->translatedFormat('d M Y, H:i') : '-' }}
                    </div>
                    <div><strong>Estimasi Selesai:</strong> {{ $serviceOrder['estimated_completion_date'] ?
                        \Carbon\Carbon::parse($serviceOrder['estimated_completion_date'])->translatedFormat('d M Y') :
                        '-' }}</div>
                    ...
                    <div><strong>Tgl Selesai Aktual:</strong> {{ $serviceOrder['date_completed'] ?
                        \Carbon\Carbon::parse($serviceOrder['date_completed'])->translatedFormat('d M Y, H:i') : 'Belum
                        Selesai' }}</div>
                    @if($serviceOrder['status'] === 'Picked Up')
                    <div><strong>Tgl Diambil:</strong> {{ $serviceOrder['date_picked_up'] ?
                        \Carbon\Carbon::parse($serviceOrder['date_picked_up'])->translatedFormat('d M Y, H:i') : '-' }}
                    </div>
                    @endif
                    @endif
                </div>
                <div class="mt-2 text-sm">
                    ...
                    <p class="text-gray-600 whitespace-pre-wrap">{{ $serviceOrder['problem_description_short'] }}</p>
                </div>
            </section>
                ...
                <div class="space-y-4">
                    ...
                    <div class="border rounded-md p-3 text-sm bg-gray-50">
                        <p class="font-semibold">{{ $update['update_type'] ?: 'Update' }} - <span
                                class="font-normal text-gray-600">{{ $update['created_at_formatted'] }}</span></p>
                        <p class="text-xs text-gray-500">Oleh: {{ $update['updated_by_name'] }}</p>
                        @if($update['status_from'] || $update['status_to'])
                        @if ($update['status_from'] !== $update['status_to']) {{-- Hanya tampilkan jika ada perubahan
                        status --}}
                        ...
                        <div class="mt-2 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                            ...
                            {{-- Link ini yang akan memicu Lightbox --}}
                            <a href="{{ asset('storage/' . $photo['file_path']) }}"
                                data-lightbox="order-{{$serviceOrder['service_order_number']}}-update-{{$loop->parent->index}}"
                                data-title="{{ $photo['caption'] ?: ($update['notes'] ?: 'Bukti Foto') }}">
            ...
            <section>
                ...
                <div class="text-sm">
                    <p><strong>Masa Berlaku:</strong> {{ $serviceOrder['warranty']['start_date_formatted'] }} s/d {{
                        $serviceOrder['warranty']['end_date_formatted'] }}</p>
