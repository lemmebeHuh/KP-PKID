...
<head>
    ...
    <title>Artikel & Tips - Pangkalan Komputer ID</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
</head>

<body class="bg-gray-100 font-sans">
    {{-- Navigasi Publik Sederhana --}}
    @include('layouts.navigation') {{-- Kita bisa buat file navigasi terpisah nanti --}}
    ...
    <div class="container mx-auto mt-6 p-4">
        ...
        <div class="mb-6 text-center">
            <p class="text-lg text-gray-700">Hasil pencarian untuk: <strong class="font-semibold">{{ request('search')
                    }}</strong></p>
            <a href="{{ route('articles.index-public') }}" class="text-sm text-indigo-600 hover:text-indigo-800">Lihat
                semua artikel</a>
        </div>
        @endif
        ...
        <div class="mb-8 max-w-lg mx-auto">
            <form action="{{ route('articles.index-public') }}" method="GET" class="flex">
                <input type="text" name="search"
                    ...
                    placeholder="Cari artikel berdasarkan judul..." value="{{ request('search') }}"> {{-- Menampilkan
                kembali term pencarian --}}
                <button type="submit"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div
                ...
                <a href="{{ route('articles.show-public', $article->slug) }}">
                    ...
                    @else
                    <img src="https://via.placeholder.com/400x250.png?text=Artikel" alt="{{ $article->title }}"
                        class="w-full h-56 object-cover">
                    @endif
                </a>
                <div class="p-6 flex-grow flex flex-col">
                    ...
                    <h3 class="mt-1 text-xl font-semibold text-gray-800">
                        <a href="{{ route('articles.show-public', $article->slug) }}" class="hover:text-indigo-800">{{
                            $article->title }}</a>
                    </h3>
                    ...
                    <div class="mt-4 flex items-center text-xs text-gray-500">
                        ...
                        <span>{{ $article->published_at ? $article->published_at->translatedFormat('d F Y') : ''
                            }}</span>
                    </div>
                </div>
            </div>
        ...
        @else
        <p class="text-gray-500 text-center py-10">Belum ada artikel yang dipublikasikan.</p>
        @endif
    </div>

    {{-- @include('layouts.public-footer') Kita bisa buat file footer terpisah nanti --}}
</body>
